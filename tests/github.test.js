...
describe('GitHub API Tests', () => {
  ...
  it('should fetch user repositories', async () => {
    const response = await axios.get(`${base_url}/repos/${owner}/${repo}`, {headers});
  ...
  it('should create a new issue', async () => {
    ...
    const response = await axios.post(`${base_url}/repos/${owner}/${repo}/issues`, body, {headers});
    expect(response.status).toBe(201);
    expect(response.data).toHaveProperty('title', 'Found a bug');
    expect(response.data).toHaveProperty('body', 'This is a test issue created by automation');
    expect(response.data).toHaveProperty('state', 'open');
  });

  it('should get issue number from new issue', async () => {
    const response = await axios.get(`${base_url}/repos/${owner}/${repo}/issues`, {headers});
    expect(response.status).toBe(200);
  ...
  it('should update an existing issue', async () => {
    ...
    const response = await axios.patch(`${base_url}/repos/${owner}/${repo}/issues/${issueNumber}`, body, {headers});
    expect(response.status).toBe(200);
    expect(response.data.labels.map(label => label.name)).toEqual(['bug', 'invalid']);
  });

  it('should get a list of repo commits', async () => {
    const response = await axios.get(`${base_url}/repos/${owner}/${repo}/commits`, {headers});
    expect(response.status).toBe(200);
